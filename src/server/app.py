...
class ZerePyServer:
    ...
    def setup_routes(self):
        ...
        async def agent_action(action_request: ActionRequest):
            ...
            try:
                # Handle None case for params
                params = action_request.params if action_request.params is not None else []
        ...
        async def configure_connection(name: str, config: ConfigureRequest):
            ...
            try:
                ...
                # Ensure params is a dictionary with string keys
                safe_params = {} if config.params is None else {str(k): v for k, v in config.params.items()}
                success = connection.configure(**safe_params)
                if success:
...
if __name__ == "__main__":
    import uvicorn
    # Only bind to port 8000 in development mode
    if os.environ.get("NODE_ENV") == "production":
        # In production, only act as an API server, don't bind to any port
        print("Running in production mode, not binding FastAPI to any port")
    else:
        # In development mode, bind to port 8000
