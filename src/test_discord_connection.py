"""
Test Discord connection for proper configuration

This script provides a minimal test of API connectivity to verify that
the Discord connection is working properly.
"""
import os
import sys
import logging
...
async def test_discord_connection():
    """Test that Discord connection configuration is available"""
    token = os.environ.get("DISCORD_BOT_TOKEN")
    if token:
        logger.info("✅ DISCORD_BOT_TOKEN is configured")
        # Only show first few characters for security
        token_preview = token[:4] + "*" * (len(token) - 8) + token[-4:]
...
async def main():
    ...
    try:
        ...
        logger.info("🔄 Testing Discord connection configuration...")
        discord_success = await test_discord_connection()
        
        # Test API connection
        logger.info("🔄 Starting API connection test...")
        api_success = await test_api_connection()
        
        if discord_success and api_success:
            ...
            logger.error(f"❌ Connection tests failed for: {', '.join(failures)}")
        
    except Exception as e:
        logger.error(f"❌ Error in main: {str(e)}")

if __name__ == "__main__":
    logger.info("Starting Discord connection tests...")
    ...
    logger.info("Discord connection test service complete")